...
- **진행 순서도**: 아이템 기획 → Python 설치 시나리오(P0, 기존 해결법 재사용) / Excel 설치 시나리오(P1, 새 해결법 학습) → 팀 공유 → 검증 → 시연. 박스를 눌러 대기·진행 중·막힘·완료를 직접 선택합니다.
...
진행 순서도의 수동 상태는 **각 브라우저에 저장**합니다. **상태 링크 공유**는 복사 시점의 모든 노드 상태를 전달하며 이후 변경은 새 링크로 공유합니다. 실시간 공동 상태 편집이 아닙니다. 이전 8단계(v1)의 상태를 새 6단계(v2)로 추측해서 옮기지 않습니다.
