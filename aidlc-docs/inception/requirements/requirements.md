...
- **NFR-RUN-1**: Windows-native, **Python** 기반. 새 clone 또는 ZIP 압축해제본에서 선언된 의존성만으로 실행 가능해야 한다(개인 PC 전역 설정 비의존).
  - **P1 Excel 시나리오 한정 예외(2026-09-08 승인, CJ)**: P1 실패 모델(Office 암호화 합성 파일 + 실행 중 Excel에 대한 허용 read-only attach)은 **Excel 설치·실행 + 사용자가 합성 파일을 열어둔 상태 + `pywin32`(win32com)**를 **명시적 사전조건**으로 선언한다. 이는 "전역 설정 비의존"의 **P1 Excel 경로 한정 예외**이며 **P0 및 그 밖의 제품 실행 범위로 확대하지 않는다**. **Excel 미설치 환경에서는 해당 P1 경로를 `NOT_RUN`으로 명확히 표시**한다(무결성·게시 게이트는 불변, FR-P1-7).
