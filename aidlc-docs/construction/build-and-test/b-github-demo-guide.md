...
현재 상태줄의 인기·실제 재사용 숫자는 **로컬 실행 기준**입니다. 수신만 한 CJ 화면의 로컬 숫자가 0이어도 조직 이벤트가 없는 뜻은 아닙니다. 공용 이벤트 확인은 조직 대시보드의 해당 Skill `org_reuse_count`/활동 또는 조직 스냅샷을 사용합니다. 팀 순위·공용 실적의 상태줄 표현 마무리는 별도 남은 작업입니다. 기존 공유 Skill의 작성자는 `local`로 기록되어 있으므로 실명으로 꾸며 바꾸지 않습니다(내용 변경은 digest와 승인 대상 변경).
