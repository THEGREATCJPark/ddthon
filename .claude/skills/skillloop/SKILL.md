...
- 설치·사용 검증이 성공하고 CLI가 `counted=True`를 반환하면 CLI가 제공한 Skill 표시명으로 “{실제로 선택된 Skill 이름} Skill로 설치와 사용 확인을 마쳤습니다. 검증된 재사용 성공 기록이 1회 추가됐습니다.”라고 짧게 알린다. 다른 Skill에 P0 이름을 붙이지 않는다. `counted=False`나 `INSTALL_OK_NO_REUSE`에서는 성공 기록이 추가됐다고 말하지 않는다. 미래형 “추가됩니다”보다 실제 완료된 결과를 전달한다.
